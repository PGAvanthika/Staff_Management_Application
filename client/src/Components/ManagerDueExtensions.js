...
import { Box, Card, CardContent, Typography, Grid, Alert, Chip, Stack, Button, Tabs, Tab, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper } from '@mui/material';
...
const ManagerDueExtensions = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/dues/manager/history?year=${year}&month=${month}`, {
        credentials: 'include',
      });
      ...
      {tab === 0 && (
        <Grid container spacing={3}>
          {dueExtensions.length === 0 && (
            <Grid item xs={12}>
              <Typography variant="body1" sx={{ m: 2 }}>No due extension requests found.</Typography>
            </Grid>
          )}
              ...
              >
                <CardContent>
                  <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                    ...
                    <Chip
                      label={due.status ? due.status.charAt(0).toUpperCase() + due.status.slice(1) : 'Pending'}
                      color={statusColor(due.status)}
                      size="small"
                      sx={{ fontWeight: 600 }}
                    />
                  </Stack>
                  <Typography color="textSecondary" fontSize={15} mb={0.5}>
                  ...
                  <Typography color="textSecondary" fontSize={15}>
                    <strong>Current Deadline:</strong> {due.current_deadline ? new Date(due.current_deadline).toLocaleDateString() : 'N/A'}
                  </Typography>
                </CardContent>
              </Card>
            </Grid>
        ...
        <TableContainer component={Paper} sx={{ mt: 2 }}>
          <Table>
            ...
            <TableBody>
              {history.length === 0 && (
                <TableRow>
                  <TableCell colSpan={9} align="center">No history for this month.</TableCell>
                </TableRow>
              )}
              {history.map((due) => (
                <TableRow key={due.due_id}>
                  ...
                  <TableCell>{due.current_deadline ? new Date(due.current_deadline).toLocaleDateString() : 'N/A'}</TableCell>
                  <TableCell>{due.reason}</TableCell>
                  <TableCell>{due.created_at ? new Date(due.created_at).toLocaleDateString() : 'N/A'}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
